...
testApp.TestModel.prototype.timersTick = function(timeNow, that) {
    ...
    if(testTimerID == recievedTimerID) {
        ...
        this.fireEvent('test:testTimerShow', this._timeNow);
    ...
    if(this.config.taskTimer == true) {
        ...
        if(taskTimerID == recievedTimerID) {
            ...
            //timeNow = this.timer.timeNow;
            this.fireEvent('test:taskTimerShow', this._timeNow);
...
//клик на сайдбар
testApp.TestModel.prototype.sidebarClick = function(id, element){
    this._id = id;
    ...
    if (this.resultMode) {
        if ($(this._element).parent().hasClass('answer-given')) {
            this.showTask(this._id, oldID, maxID);
        }
...
testApp.TestModel.prototype.showTask = function(id) {
    ...
    if(this.__proto__.__proto__.fireEvent('test:showTask', data)) {
        console.log('test:showTask FIRED!!!', this);
    } else {
        console.log('EVENT DIDN\'T FIRE!', this.__proto__.__proto__);
    }
...
testApp.TestModel.prototype.showPrevTask = function() {
    ...
    //клик на ответ
testApp.TestModel.prototype.giveAnswer = function(id, answer) {
    ...
    this.init = function(attrs) {
        ...
        if(attrs.config) {
            for(property in attrs.config) {
                if(!attrs.config.hasOwnProperty(property)) continue;
